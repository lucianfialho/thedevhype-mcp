...
- Content is always markdown. Excerpts are auto-generated.`,
  tools: [
    ...
    { name: 'edit_entry', description: 'Update the content, title, or tags of an existing entry. Requires the entry ID from list_entries or search. Replaces the content entirely — send the full updated markdown, not a diff. Excerpt is auto-regenerated. Idempotent.' },
    { name: 'read_entry', description: 'Read the full content of an entry by its ID. Returns all fields: type, title, content (full markdown), URL, source, tags, and timestamps. Use when you need the complete text — list_entries and search only return excerpts. Read-only.' },
    { name: 'delete_entry', description: 'Permanently delete an entry and all its connections. Requires the entry ID. Destructive — this cannot be undone. Both incoming and outgoing connections are removed.' },
    ...
    { name: 'search', description: 'Full-text search across all entries using PostgreSQL websearch_to_tsquery. Supports natural language queries. Filter by entry type. Returns up to 20 results with ID, type, title, URL, source, excerpt, tags, and relevance rank. Read-only.' },
    { name: 'list_entries', description: 'Browse entries with optional filters by type and tag. Returns entries ordered by last updated. Use to explore the knowledge base or find entries to connect. Read-only. Returns ID, type, title, URL, source, excerpt, tags, and timestamps.' },
    { name: 'connect', description: 'Create a bidirectional connection between two entries. Use to build a knowledge graph — e.g. connect a person to a company, a note to a link, a highlight to a person. Provide an optional note describing the relationship. Idempotent — connecting already-connected entries is a no-op.' },
    { name: 'list_connections', description: 'List all entries connected to a given entry. Returns the connected entries with their type, title, and the connection note. Use to explore the knowledge graph from a starting point. Read-only.' },
  ],
  init: (server) => {
    // ─── create_note ───
    server.tool(
